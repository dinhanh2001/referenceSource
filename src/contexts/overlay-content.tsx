...
export const OverlayProvider = memo(({ children }: Props) => {
  ...
  const dialogResolveRef = useRef<(val: boolean | undefined) => void>();
  ...
  const resolveDialog = useCallback((val: boolean | undefined) => {
      ...
      <Modal isVisible={dialogVisible} animationIn="fadeIn" animationOut="fadeOut">
        <View style={tw`flex-1 justify-center items-center`}>
          <View style={tw`w-full rounded-4px bg-white p-20px`}>
            {dialog?.closeButtonIconShown && (
              <TouchableOpacity
                style={[tw`absolute right-20px top-20px z-10`]}
                onPress={() => resolveDialog(undefined)}
              >
                <CloseCircleSVG />
              </TouchableOpacity>
            )}
