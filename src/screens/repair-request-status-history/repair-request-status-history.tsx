...
type ScreenRouteProp = RouteProp<AppStackNavigatorParamList, AppRoutes.REPAIR_REQUEST_STATUS_HISTORY>;

export const RepairRequestStatusHistory = () => {
  const {
    params: { bookingId },
  } = useRoute<ScreenRouteProp>();

  const { data: booking } = usePartnerBookingQuery({ variables: { id: bookingId } });
  const { data, loading, refetch } = useBookingStatusHistoryQuery({ variables: { bookingId } });
  const { isRefetchingByUser, refetchByUser } = useRefreshByUser(refetch);

  const { code, createdAt: bookingCreated } = booking?.partnerBooking || {};
