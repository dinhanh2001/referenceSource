...
import { AppHeader, Screen, TextArea, tw, UploadMediaInput } from '../../components';
import { useFullScreenLoading, useOverlay } from '../../contexts';
import { usePartnerCreateServiceFeedbackMutation } from '../../graphql/mutations/partnerCreateServiceFeedback.generated';
import { PartnerGetCountStatusServiceFeedbackDocument } from '../../graphql/queries/partnerGetCountStatusServiceFeedback.generated';
import { PartnerGetServiceFeedbacksDocument } from '../../graphql/queries/partnerGetServiceFeedbacks.generated';
...
import { typeIcon } from '../feedback/components';

import { convertServiceFeedbackTypeEnum } from './components/enum';
...
export const FeedbackCreate = React.memo(() => {
  ...
  const [createServiceFeedback, { loading: creating }] = usePartnerCreateServiceFeedbackMutation({
    onError(error) {
      showMessage({
        type: 'danger',
  ...
  const onSend = useCallback(
    async (values: FormData) => {
      ...
      createServiceFeedback({
        ...
        refetchQueries: [PartnerGetServiceFeedbacksDocument, PartnerGetCountStatusServiceFeedbackDocument],
      });
    },
  ...
  return (
    <Screen>
      ...
      <KeyboardAwareScrollView contentContainerStyle={tw`p-16px grow`}>
        ...
        />
        {errors.media?.message != null && <Text style={tw`text-12px text-error mt-8px`}>{errors.media?.message}</Text>}
