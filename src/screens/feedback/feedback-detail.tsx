...
import { usePartnerGetServiceFeedbackQuery } from '../../graphql/queries/partnerGetServiceFeedback.generated';
...
import { convertServiceFeedbackTypeEnum, convertServiceFeedbacksStatusEnum } from './components/enum';

export const FeedbackDetail = React.memo(
  ({ route }: NativeStackScreenProps<AppStackNavigatorParamList, AppRoutes.FEEDBACK_DETAIL>) => {
    const { id = '' } = route.params;

    const { bottom } = useSafeAreaInsets();

    const { data, loading, refetch } = usePartnerGetServiceFeedbackQuery({ variables: { id } });
    const { isRefetchingByUser, refetchByUser } = useRefreshByUser(refetch);

    const feedback = useMemo(() => data?.partnerGetServiceFeedback, [data]);
