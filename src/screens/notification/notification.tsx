...
import { NotificationSVG, ReceiptCustomSVG, RepairSVG } from '../../svg';
...
export const Notification = React.memo(() => {
  ...
  const { data, refetch: refetchBadge } = usePartnerNotificationTypeUnSeenCountQuery();
  ...
  const tabBarLabel = useCallback(
    (type: NotificationTypeEnum) =>
      ({ focused }: tabBarLabelProps) => {
        ...
        const count = data?.partnerNotificationTypeUnSeenCount?.find?.((e) => e?.type === type)?.count || 0;
    ...
    [data?.partnerNotificationTypeUnSeenCount, width],
  );
