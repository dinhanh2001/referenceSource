...
import { IHomeActionPayload, INofifyState, IRequestActionPayload } from 'types';
...
function* onGetBaseActionsRequested(action: IHomeActionPayload) {
  try {
    ...
    if (action.payload.endPoint === API_ENDPOINT.HOME.CATELOG) {
      // console.log('res====', rs);
    }
    if (rs.status === 200) {
      ...
      let payload = {};
      if (dataKey && rs?.data?.result) {
        payload = {
          [`${dataKey}`]: rs?.data?.result,
        };
      }
