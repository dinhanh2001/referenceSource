...
const FindCar = () => {
  ...
  const [isShowRecentLocation, setIsShowRecentLocation] = useState(false);
  const [locationToFinal, setLocationToFinal] = useState();
  ...
  useEffect(() => {
    const { from, to, user_location } = fromToData;
  ...
  useEffect(() => {
    setIsShowRecentLocation(!!route?.params?.isShowRecentLocation);
  }, [route?.params?.isShowRecentLocation]);
  ...
  const [deliveryDriverSelected, setDeliveryDriverSelected] = useState<
    IDeliveryOption | undefined
  >();

  useEffect(() => {
    const filteredType = route?.params?.type;
    const filteredOptions =
      filteredType !== 'CAR_DRIVER'
        ? deliveryDriverOptions.filter(
            elm => elm?.type.includes(filteredType) && elm?.vehicle,
          )
        : deliveryDriverOptions.filter(elm => !elm?.vehicle);
    if (deliveryDriverSelected === undefined && filteredOptions?.length) {
      let minPrice = filteredOptions?.[0]?.price;
      let minIndex = filteredOptions?.findIndex(d => d?.price < minPrice);
      if (minIndex < 0) {
        minIndex = 0;
      }
      if (filteredOptions) {
        setDeliveryDriverSelected(filteredOptions?.[minIndex]);
      }
    }
  }, [deliveryDriverOptions, route?.params?.type, deliveryDriverSelected]);
  ...
  const snapPointModal = useMemo(() => {
    ...
    return rs;
  }, [stepView, heightChooseWayDelivery, heightSheet, isShowRecentLocation]);

  const refTimeOutCacheRunningFunction = useRef<ReturnType<
    typeof setTimeout
  > | null>(null);

  const onChooseFromToChange = useCallback((fromData, toData, province) => {
    setFromToData({
      from: fromData,
      to: toData,
  ...
  const renderContentModal = (step: FindCarScreenStepView) => {
    switch (step) {
        ...
        if (isShowRecentLocation) {
          return (
            <RecentLocation
              ...
              onMap={() => setIsShowRecentLocation(false)}
              setStepView={setStepView}
              setIsShowRecentLocation={setIsShowRecentLocation}
              setLocationFinal={setLocationToFinal}
            />
          );
        }
        return (
        ...
        return (
          <>
            <ChooseFromTo
              ...
              locationToFinal={locationToFinal}
      ...
      case FindCarScreenStepView.CANNOT_FOUND_DRIVER:
        return <CannotFoundDriver title={route?.params?.title || ''} />;
