...
import { getProvinceFromAutocomplete } from 'utils/addressUtils';
...
const ChooseFromTo = React.forwardRef<{}, IProps>((props, ref) => {
  ...
  const { user } = useAuth();
  const timeOutToLocation = useRef();
      ...
      onNameByLatLng(
        ...
        result => {
          searchAutoComplete(
            ...
            res => {
              setProvince(getProvinceFromAutocomplete(res?.[0]?.terms || []));
              setFromLocation({ ...res[0], address: res[0]?.description });
              searchDetail({ place_id: res[0].place_id }, a => {
                if (a.result) {
          ...
          onNameByLatLng(
            ...
            result => {
              searchAutoComplete(
                ...
                res => {
                  setProvince(
                    getProvinceFromAutocomplete(res?.[0]?.terms || []),
                  );
                  setFromLocation({ ...res[0], address: res[0]?.description });
                },
              );
  ...
  useEffect(() => {
    if (props?.locationToFinal) {
      setToLocationFinal(props?.locationToFinal);
      setToLocation(props?.locationToFinal);
    }
  }, [props?.locationToFinal]);
  ...
  const changeFromToFinal = useCallback(() => {
    props.onChooseFromToChange?.(fromLocationFinal, toLocationFinal, province);
  }, [
    toLocationFinal,
    fromLocationFinal,
    province,
    props.onChooseFromToChange,
  ]);
          ...
          <ViewCus
            ...
            <Buttons
              ...
              onPress={async () => {
                ...
                const recentLocations = await AsyncStorage.getItem(
                  'RecentLocations',
                );
                const recentLocationsParse = JSON.parse(
                  recentLocations ?? '[]',
                );
                ...
                const dataRecentLocations = [
                  toLocationFinal,
                  ...recentLocationsParse,
                ];
                await AsyncStorage.setItem(
                  'RecentLocations',
                  JSON.stringify(dataRecentLocations),
                );
