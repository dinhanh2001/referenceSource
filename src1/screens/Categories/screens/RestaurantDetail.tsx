...
const RestaurantDetail: React.FC = () => {
  ...
  const onAddMenu = foodInfo => {
    const itemCart = carts?.find(cart => cart?.itemId === foodInfo?.id);

    if (!isEmpty(itemCart)) {
      const form = {
        orderItems: {
          ...itemCart,
          quantity: Number(itemCart?.quantity || 1) + 1,
          index: itemCart?.index,
        },
        extraOptions: itemCart?.extraOptions,
      };
      ...
      const form = {
        orderItems: {
          itemId: foodInfo?.id,
          quantity: 1,
          itemName: foodInfo?.food_name,
          image: foodInfo.images?.[0],
  ...
  return (
    <HomeLayout
      ...
      <StatusBar barStyle={EStatusBar.LIGHT} />
