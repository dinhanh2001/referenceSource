...
export const useAuth = () => {
    ...
    async (formData: IFormDataLogin) => {
      dispatch(
        UserActions.postBaseActionsRequest(
          ...
          async res => {
            console.log('rsss login', res);
            const check = res?.status;
            if ([200, 406].includes(check)) {
              const data = check === 406 ? res?.data : res?.data?.result?.[0];
              const r = check === 406 ? Routes.InputPassword : Routes.OTP;
              ...
              danger(t('error'), `Đăng nhập không thành công`);
            }
          },
        ),
      );
        ...
        UserActions.postBaseActionsRequest(
          ...
          res => {
            if (res?.status === 200) {
              ...
              danger(t('error'), 'Sai mật khẩu');
            }
          },
        ),
      );
    },
    [dispatch, saveKeyStore, getProfileUser, danger, t],
  );
  ...
  const onUploadImageChat = useCallback(
    (file: any, token: any, callback: (result: any) => void) => {
      ...
      axios
        .post(
          `${Config.API_HOST}${API_ENDPOINT.AUTH.UPLOAD_IMAGE}`,
          _formData,
          {
            headers: {
              'Content-Type': 'multipart/form-data',
  ...
  // const setLoadingInitApp = useCallback(()=>{
