...
export const NotificationList = () => {
  ...
  const { data, loading, refetch, fetchMore } = useUserNotificationsQuery({
    variables: {
      type,
  ...
  const onLoadMore = useCallback(() => {
    if (data && currentPage.current < data?.userNotifications.meta.totalPages && !loading) {
      currentPage.current += 1;
      try {
        setLoadingMore(true);
        fetchMore({
  ...
  const onPressItem = useCallback(
    (item: NotificationEntity) => {
      ...
      switch (type) {
        case NotificationTypeEnum.ORDER:
          navigation.navigate('e-commerce/my-order-detail', { orderId: objectId as string });
          break;

        case NotificationTypeEnum.MAINTENANCE:
          navigation.navigate('maintenance/detail', { maintenanceId: objectId as string });
          break;

        case NotificationTypeEnum.BOOKING:
          navigation.navigate('my-repair-request/detail', { bookingId: objectId as string });
          break;
  ...
  if (loading && !data?.userNotifications?.items?.length) {
  ...
  return (
    <FlashList
      ...
      data={data?.userNotifications?.items as NotificationEntity[]}
