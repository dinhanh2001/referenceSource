...
export const SurveyListScreen = () => {
  ...
  const { data, loading, refetch, fetchMore } = useUserGetSurveysQuery({
    variables: {
  ...
  const onLoadMore = useCallback(() => {
    if (data && currentPage.current < data?.userGetSurveys.meta.totalPages && !loading) {
      currentPage.current += 1;
      try {
        setLoadingMore(true);
        fetchMore({
  ...
  const onPressDetail = useCallback(
    (surveyId: string) => {
      navigation.navigate('form-survey', { surveyId });
    },
    [navigation],
  );
  ...
  if (loading && !data?.userGetSurveys?.items?.length) {
    return <ActivityIndicator />;
  }

  return (
    <FlatList
      data={data?.userGetSurveys?.items as SurveyEntity[]}
